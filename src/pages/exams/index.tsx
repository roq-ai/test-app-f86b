...
import { getExams, deleteExamById } from 'apiSdk/exams';
import { ExamInterface } from 'interfaces/exam';

type ColumnType = ColumnDef<ExamInterface, unknown>;
...
export function ExamListPage(props: ExamListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ExamInterface>>(
    () => `/exams?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ExamInterface) => {
    if (hasAccess('exam', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/exams/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/exams/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
          ...
          {hasAccess('exam', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/exams/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('exam', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="exam" />
        </Flex>

        {hasAccess('exam', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/exams/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
